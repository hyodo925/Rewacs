...
from swg_navigation import SWGNavigation
...
from algo.swg.trainer import SWG
from torch.optim.lr_scheduler import CosineAnnealingLR
from algo.swg.diffusion import SelfWeighted_GaussianDiffusion
from algo.swg.helpers import EMA
from algo.swg.model import MLP
try:
    import wandb
...
config_path = "./configs/swg_config.py"
...
model = MLP(state_dim=projection_dim, action_dim=action_dim, device=device,integrator=actor_integrator)
diffusion_model = SelfWeighted_GaussianDiffusion(
    model=model, 
    data_dim=action_dim + 1, **actor_params.diffusion
)
ema = EMA(ema_decay)
ema_model = copy.deepcopy(diffusion_model)

q_integrator = EmbeddedGaussianIntegrator(
    cfg.model.obs_dim,
    cfg.model.r_obs_dim,
    projection_dim=cfg.model.projection_dim,
...
q = SocialCritic(
    ...
    integrator=q_integrator,
    single=False,
)

value_integrator = EmbeddedGaussianIntegrator(
    cfg.model.obs_dim,
    cfg.model.r_obs_dim,
    projection_dim=cfg.model.projection_dim,
    enc_hdims=cfg.model.critic_integrator_enc_hdims,
...
value = SocialCritic(
    ...
    1,
    h_dims=cfg.model.critic_h_dims,
    integrator=value_integrator,
    single=True,
)

model = SWGNavigation(diffusion_model=diffusion_model, q=q, value=value)
...
q_optimizer = torch.optim.Adam(model.q.parameters(), lr=cfg.train.lr)
value_optimizer = torch.optim.Adam(model.value.parameters(), lr=cfg.train.lr)
actor_optimizer = torch.optim.AdamW(model.actor.parameters(), lr=cfg.train.lr, weight_decay=cfg.train.train_params.weight_decay,)
scheduler = CosineAnnealingLR(
    actor_optimizer, T_max=n_steps, eta_min=train_params.eta_min
)
...
with tqdm(range(cfg.train.total_it), desc=trainer.alg_name + " Training") as pbar:
    ...
    trainer.build_weights()

    for step in tqdm(range(1, n_steps + 1), smoothing=0.1):
        trainer.update_actor()

        if step % train_params.update_ema_every == 0:
            with torch.no_grad():
                trainer.step_ema(step)
