...
def eval_policy(
    ...
    for i in tqdm(range(eval_episodes)):
        ...
        input_a = torch.empty(1, 1, 2)
        act = torch.zeros_like(input_a)
        rwd = 0
        # robot_obs, human_obs = robot_obs/4., human_obs/4.
        # state = psr.state0.expand(5, 20).to(psr.device)
        while not done:
            trainer.infer_posterior_single_task(
                (
                    humans_obs.unsqueeze(0).to(model.device),
                    robot_obs.reshape(1, 1, -1).to(model.device),
                ),
                act.to(model.device),
                torch.tensor([[np.float32(rwd)]]).to(model.device),
            )
            task_z = trainer.z
            task_z = [z.repeat(1, 1) for z in task_z]
            _, _, action  = model.generate_action_z(
                (
                    humans_obs.unsqueeze(0).to(model.device),
                    robot_obs.reshape(1, 1, -1).to(model.device),
                ),
...
def eval_policy_fql(
    ...
    for i in tqdm(range(eval_episodes)):
        ...
        while not done:
            ...
            action, _= model.generate_action_one_step(
                (
                    humans_obs.unsqueeze(0).to(model.device),
                    robot_obs.reshape(1, 1, -1).to(model.device),
                ),
