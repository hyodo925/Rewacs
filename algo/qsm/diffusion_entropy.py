...
class Diffusion(nn.Module):
    def __init__(self, state_dim, action_dim, model, max_action,
        ...
        thetas_cumsum = torch.cumsum(betas, dim=0)
    ...
    # ------------------------------------------ training ------------------------------------------#
    def entropy(self,x0):
        self.thetas_bar = self.thetas_bar.to(x0.device)
        batch_size = len(x0)
        t1 = torch.full((batch_size,), 1, device=x0.device, dtype=torch.long)
        x1 = self.q_sample(x0,t1)
        xT = torch.randn_like(x0)
        
        return torch.exp(self.log_forward_transition(x1,xT,t1,self.n_timesteps-1) + self.log_forward_transition(x0, x1, 0, t1) - self.log_forward_transition(x0, xT, 0, self.n_timesteps-1))
