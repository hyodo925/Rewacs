...
class SocialActorMetaCriticAWAC(nn.Module):
    ...
    def sample(self, data):
        ...
        action = dist.rsample()
        log_prob = dist.log_prob(action).sum(axis=-1, keepdim=True)
        #if you sample action and then adopt tanh to normalize you should calibulate the log_prob based on action bound in SAC paper
...
class SocialActorMetaCriticSAC(nn.Module):
    def __init__(
        self,
        D,
        d,
