...
class MetaCriticAWAC:
    ...
    def update(self, update_actor=False, updater=None, data_for_logging=None):
        ...
        ### compute actor loss      ###
        ###############################
        action_gen, log_prob, _, _, other_output = self.model.actor.sample((obs.to(self.device),r_obs.reshape(-1, 1, self.r_obs_dim).to(self.device)),return_other_output=True,)
        with torch.no_grad():
        ...
        loss_act = -(self.model.actor.get_log_prob((obs.to(self.device),r_obs.reshape(-1, 1, self.r_obs_dim).to(self.device),),act.squeeze().to(self.device),)* weights).mean()

        ###############################
        ### compute loss aux        ###
        ###############################
        concat_output = torch.cat([other_output.to(self.device), obs.reshape(-1, self.obs_dim * self.human_num).to(self.device), r_obs.to(self.device), act.to(self.device)],1)

        
        loss_auxiliary = self.model.feature_critic(concat_output)

        ################################################
        ### first pseudo update with loss act        ###
        ################################################

        grads_critic = torch.autograd.grad(loss_act, self.model.actor.parameters(), create_graph=True)
        updater.step(self.model.actor, grads_critic, "phi_old", self.policy_lr)
        old_param = updater.get("phi_old")
        
        with torch.no_grad():
            qw_ref = torch.min(torch.cat((Q1, Q2), 1), dim=1)[0].reshape((-1, 1))
            pi_val, log_pi_val, *_ = self.model.actor.sample_with_params((obs_val.to(self.device),r_obs_val.reshape(-1, 1, self.r_obs_dim).to(self.device),),params=old_param,)
            v_act1, v_act2 = self.model.critic((obs_val.to(self.device), r_obs_val.to(self.device)),act=pi_val)
            qw_gen = torch.min(torch.cat((v_act1, v_act2), 1), dim=1)[0].reshape((-1, 1))
            adv = torch.max(torch.zeros_like(qw_ref), qw_ref - qw_gen)
            weights = self.thexp(adv / self.beta)

        policy_loss_val = -(self.model.actor.get_log_prob((obs_val.to(self.device),r_obs_val.reshape(-1, 1, self.r_obs_dim).to(self.device),),pi_val.squeeze().to(self.device),)* weights).mean()
        ...
        updater.step(self.model.actor, grads_mcritic, "phi_new", self.policy_lr, from_params=old_param)
        new_param = updater.get("phi_new")
        pi_val_new, log_pi_val_new, *_ = self.model.actor.sample_with_params((obs_val.to(self.device),r_obs_val.reshape(-1, 1, self.r_obs_dim).to(self.device),),params=new_param)
        v_act1, v_act2 = self.critic((obs_val.to(self.device), r_obs_val.to(self.device)),act=pi_val_new)

        qw_gen = torch.min(torch.cat((v_act1, v_act2), 1), dim=1)[0].reshape((-1, 1))
        adv = torch.max(torch.zeros_like(qw_ref), qw_ref - qw_gen)
        weights = self.thexp(adv / self.beta)

        policy_loss_val_new = -(self.model.actor.get_log_prob((obs_val.to(self.device),r_obs_val.reshape(-1, 1, self.r_obs_dim).to(self.device)),pi_val_new.squeeze().to(self.device),)* weights).mean()
