...
class ExplorerCrowdSim:
    ...
    def exploration_k_ep_with_flow(
        ...
        for _ in range(k):
            ...
            while not done:
                ...
                switches.append(torch.as_tensor([float(switching_necessity)], dtype=torch.float32))
                # switching_necessity = True
    ...
    def exploration_k_ep_with_switching(
        self,
        buffer,
        buffer_val,
        model=None,
        ...
        for _ in range(k):
            ...
            while True:
                ...
                if switching_necessity:
                    a = a_o
                    anomaly += 1
                    action_sources.append("rule")
                    if render_switching:
                        switching_list.append(True)
                else:
                    a = a_l
                    normality += 1
                    action_sources.append("learning")
                    if render_switching:
                        switching_list.append(False)
            ...
            if isinstance(info, ReachGoal) or isinstance(info, Collision):
                for i in range(len(obs)):
                    ...
                    if action_sources[i] == "learning":
                        buffer.add(samples)
                    else:
                        buffer_val.add(samples)
        if pbar:
            pbar.set_postfix(Reward=str(sum_rewards / k))
