...
from rewacs.envs.utils.action import ActionXYW
from rewacs.envs.utils.human import Human
...
from rewacs.envs.utils.state import tensor_to_joint_state
from rewacs.envs.utils.utils import point_to_segment_dist
...
class CrowdSim:
    ...
    def generate_human(self, human=None):
        ...
        assert self.current_scenario in [
            "circle_crossing",
            "square_crossing",
            "corridor",
            "monotonic",
        ...
        if self.current_scenario == "circle_crossing":
            ...
            # human.set(px, py, -px/norm_g*human.v_pref*self.time_limit, -py/norm_g*human.v_pref*self.time_limit, 0, 0, 0)
            ...
            human.set(px, py, px + gx_noise, -py + gy_noise, 0, 0, 0)

        elif self.current_scenario == "monotonic":
            # if np.random.random() > 0.5:
            assert self.human_num == 1, (
                "pedestroam number should be 1 in monotonic scenario"
            )

            sx_noise = (np.random.random() - 0.5) * human.v_pref
            sy_noise = (np.random.random() - 0.5) * human.v_pref
            ...
            # gx_noise = 0
            # gy_noise = 0
            human.set(0 + sx_noise, 4 + sy_noise, 0 + gx_noise, -4 + gy_noise, 0, 0, 0)
    ...
    def reset(self, phase="test", test_case=None):
        ...
        if self.robot.kinematics == "holonomic":
            self.robot.set(
                0, -self.circle_radius, 0, self.circle_radius, 0, 0, 0, np.pi / 2
            )
            self.goal_dist = Normal(
                torch.tensor([0, self.circle_radius]),
                torch.tensor([0.3, 0.3]),
            )
            # self.robot.set(
            #     0, -self.circle_radius, self.circle_radius, 0, 0, 0, np.pi / 2
            # )
        else:
            self.robot.set(
                0,
        ...
        return self.robot.get_full_state(), ob
    ...
    def step(self, action, update=True):
        ...
        for i, human in enumerate(self.humans):
            ...
            if self.robot.kinematics == "holonomic":
                ...
                if isinstance(action, ActionXYW):
                    theta = (self.robot.theta + action.vw * self.time_step) % (
                        2 * np.pi
                    )
                else:
                    theta = self.robot.theta
                r_vx = action.vx * np.cos(theta) - action.vy * np.sin(theta)
                r_vy = action.vx * np.sin(theta) + action.vy * np.cos(theta)
                vx = human.vx - r_vx
                vy = human.vy - r_vy
            else:
            ...
            if closest_dist < 0:
                collision = True
                logging.debug(
                    f"Collision: distance between robot and p{human.id} is {closest_dist:.2E} at time {self.global_time:.2E}"
                )
                break
        ...
        return self.robot.get_full_state(), ob, reward, done, info
    ...
    def render(
        ...
        if mode == "traj":
            ...
            robot_legend_marker = mlines.Line2D(
                [robot_positions[0][0]],
                [robot_positions[0][1]],
                color=robot_color,
                marker="o",
            ...
            plt.legend([robot_legend_marker], ["Robot"], fontsize=14)
            if output_file:
            ...
            plt.legend([robot_legend_marker, goal], ["Robot", "Goal"], fontsize=14)
            if output_file:
                plt.savefig(output_file)
            else:
                plt.show()
            ...
            # add time annotation
            time = plt.text(0.4, 0.9, f"Time: {0}", fontsize=16, transform=ax.transAxes)
            ax.add_artist(time)
            ...
            def update(frame_num):
                ...
                if not switching_list == None:
                    if switching_list[global_step]:
                        robot.fill = True
                        # robot_legend_marker.fillstyle = "full"
                        # robot.hatch = "*"
                        # ax.patch.set_facecolor("linen")
                        # ax.patch.set_alpha(0.6)
                    else:
                        robot.fill = False
                        # robot_legend_marker.fillstyle = "none"
                        # robot.fill = False
                        # robot.hatch = "none"
                ...
                time.set_text(f"Time: {frame_num * self.time_step:.2f}")
            ...
            def print_matrix_A():
                ...
                print("   " + " ".join([f"{i - 1:>5}" for i in range(w)]))
                for i in range(h):
                    print(
                        f"{i - 1:<3}"
                        + " ".join(
                            [f"{self.As[global_step][i][j]:.3f}" for j in range(w)]
                        )
                    )
                # with np.printoptions(precision=3, suppress=True):
                #     print('A is: ')
