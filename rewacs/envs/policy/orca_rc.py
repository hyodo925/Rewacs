...
from rewacs.envs.policy.policy import Policy
...
class ORCARC(Policy):
    ...
    def predict(self, state):
        ...
        self.sim.doStep()
        vx_orca = self.sim.getAgentVelocity(0)[0]
        vy_orca = self.sim.getAgentVelocity(0)[1]
        theta = robot_state.theta

        vx_r = vx_orca * np.cos(theta) + vy_orca * np.sin(theta)
        vy_r = -vx_orca * np.sin(theta) + vy_orca * np.cos(theta)

        action = ActionXY(vx_r, vy_r)
