...
from metarl.models import SocialActorMetaAWAC, SocialMetaCritic
from metarl.trainer import MetaAWAC
from models.metarl_navigation import MetaRLNavigation
...
from utils.aggregators import EmbeddedGaussianAggregator, SimpleConcatenateObsAggregator
...
from utils.models_matsumoto import (
    SocialCritic,
)
...
# run_dir = "wandb/run-20260210_194954-u59ib0ya" #with meta critic
# run_dir = "wandb/run-20260210_212054-mommjr8i" #offline_meta_critic_pre-training weight clipping SGD
# run_dir = "wandb/run-20260210_205717-m155u6be" #AWAC 
# run_dir = "wandb/run-20260210_225413-wz86n47a" #offline meta critic ASGD no weight clipping
run_dir = "wandb/run-20260210_231637-747se370" #offline meta critic ASGD actor weight clipping
model_path = os.path.join(run_dir, "files/trained_models/model_1000.pth")
# Settings
...
config_path = "./configs/meta_awac_config.py"
...
actor_aggregator = EmbeddedGaussianAggregator(
    cfg.model.obs_dim,
    cfg.model.r_obs_dim,
    projection_dim=cfg.model.projection_dim,
    enc_hdims=cfg.model.actor_aggregator_enc_hdims,
)

actor = SocialActorMetaAWAC(
    ...
    penultimate_norm=cfg.model.actor_penultimate_norm,
)

critic_aggregator = EmbeddedGaussianAggregator(
    cfg.model.obs_dim,
    cfg.model.r_obs_dim,
    projection_dim=cfg.model.projection_dim,
    enc_hdims=cfg.model.critic_aggregator_enc_hdims,
)
...
critic = SocialCritic(
    ...
    penultimate_norm=cfg.model.critic_penultimate_norm,
    single=False,
)

meta_critic_aggregator = EmbeddedGaussianAggregator(
    cfg.model.obs_dim,
    cfg.model.r_obs_dim,
    projection_dim=cfg.model.projection_dim,
    enc_hdims=cfg.model.critic_aggregator_enc_hdims,
...
# meta_critic_aggregator = SimpleConcatenateObsAggregator(
#     cfg.model.obs_dim,
#     cfg.model.r_obs_dim,
# )


meta_critic = SocialMetaCritic(
    cfg.model.actor_h_dims[-1] + cfg.model.projection_dim + cfg.model.action_dim,
    # cfg.model.actor_h_dims[-1]
    # + cfg.model.obs_dim * 5
    # + cfg.model.r_obs_dim
    # + cfg.model.action_dim,
    1,
    h_dims=cfg.model.critic_h_dims,
    aggregator=meta_critic_aggregator,
    penultimate_norm=cfg.model.meta_critic_penultimate_norm,
)
...
meta_critic_optimizer = torch.optim.ASGD(
    model.meta_critic.parameters(), lr=cfg.train.lr
)
...
trainer = MetaAWAC(
    ...
    meta_critic_optimizer=meta_critic_optimizer,
...
val_logs = eval_policy(
    ...
    phase="val",
    render=False,#cfg.eval.val_render,
    render_type=cfg.eval.render_type,
    print_results=True,
    path=None,#f"trajs/awac_meta_critic_finetuning/seed{str(cfg.train.random_seed)}/human{str(cfg.sim.human_num)}/{cfg.train.finetune_mode}/itr0",
)
if cfg.log.wandb:
    wandb.log(
        {
...
max_cdr = float("-inf")
with tqdm(
    range(cfg.train.finetune_itr), desc=trainer.alg_name + " Training", dynamic_ncols=True
) as pbar:
