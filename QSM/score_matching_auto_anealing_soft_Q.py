...
class ScoreMatchingLearner(object):
    def __init__(self,
        ...
        if not eval: 
            ...
            # set the wandb project where this run will be logged
                project="QSM-Soft-Q-Auto-Annealing-epoch",
                config={
                "learning_rate": lr,
                "architecture": "MLP",
                "dataset": "Crowd_sim",
    ...
    def return_next_entropy(self, next_state,global_steps=None, k_sample=None):
        """
        ...
        next_state_expanded = next_state.unsqueeze(0).expand(k_sample, -1, -1).reshape(-1, next_state.shape[-1])
        qi_1, qi_2 = self.critic(next_state, action)
        Q = torch.min(qi_1, qi_2)

        qi_1_noise, qi_2_noise = self.critic(next_state_expanded, noise_flat)
        ...
        wandb.log({"Q_entropy": entropy.mean()},step=global_steps)
        wandb.log({"Next_Q": Q.mean()}, step=global_steps)
        return entropy
    def return_entropy(self, state,global_steps=None, k_sample=None):
        """
        noisy_actionsに対して、log(sum(exp(Q))) の勾配を計算する関数
    ...
    def train(self, iterations,batch_size=100,global_steps=None):
        ...
        # for state, action, reward, next_state, mask,done in data_loader:
            ...
            Q_entropy = self.return_next_entropy(next_state,global_steps=global_steps)
            """ Q Training """
            ...
            target = (reward.to(self.device) + mask.to(self.device) * (target_q + Q_entropy)).detach()
            ...
            """update alpha"""
            entropy = self.return_entropy(state,global_steps=global_steps)
            target = torch.ones_like(entropy) * 1/self.M * self.target_ent
            loss_ent = F.mse_loss(entropy.detach(), target)
