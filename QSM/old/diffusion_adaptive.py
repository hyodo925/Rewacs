...
class Diffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, s, alpha=None,jacobian_norms=None):
        if not alpha == None:
            model_norm = torch.norm(self.model(x, t, s),dim=1)
            alpha = alpha.view(-1, 1) 
            jacobian_norms = jacobian_norms.view(-1, 1) 
            model_norm = model_norm.view(-1, 1)
            x_recon = self.predict_start_from_noise(x, t=t, noise=alpha*(jacobian_norms / model_norm)*self.model(x, t, s))
        else:
    ...
    # @torch.no_grad()
    def p_sample(self, x, t, s, alpha=None, jacobian_norms=None):
        b, *_, device = *x.shape, x.device
        if not alpha == None:
            model_mean, _, model_log_variance = self.p_mean_variance(x=x, t=t, s=s, alpha=alpha,jacobian_norms=jacobian_norms)
        else:
    ...
    # @torch.no_grad()
    def p_sample_loop(self, state, shape, alpha=None,jacobian_norms=None, verbose=False, return_diffusion=False):
        ...
        for i in reversed(range(0, self.n_timesteps)):
            ...
            if not alpha == None:
                x = self.p_sample(x, timesteps, state, alpha=alpha,jacobian_norms=jacobian_norms)
            else:
    ...
    def alpha_sample(self, state, alpha,jacobian_norms, *args, **kwargs):
        batch_size = state.shape[0]
        shape = (batch_size, self.action_dim)
        action = self.p_sample_loop(state, shape, alpha=alpha, jacobian_norms=jacobian_norms, *args, **kwargs)
        return action.clamp_(-self.max_action, self.max_action)
    # ------------------------------------------ training ------------------------------------------#
