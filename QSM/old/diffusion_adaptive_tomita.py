...
def compute_jacobian(critic, state, actions):
    actions = actions.detach().requires_grad_(True)
    value1,value2 = critic(state, actions)
    value1 = value1.sum()
    value2 = value2.sum()
...
class Diffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, s, alpha=None,jacobian=None):
        if not alpha == None:
            model_norm = torch.norm(self.model(x, t, s),dim=1)
            alpha = alpha.view(-1, 1) 
            # jacobian_norms = jacobian_norms.view(-1, 1) 
            # model_norm = model_norm.view(-1, 1)
            x_recon = self.predict_start_from_noise(x, t=t, noise=-alpha*jacobian)# / model_norm)*self.model(x, t, s))
        else:
    ...
    # @torch.no_grad()
    def p_sample(self, x, t, s, alpha=None, jacobian=None):
        b, *_, device = *x.shape, x.device
        if not alpha == None:
            model_mean, _, model_log_variance = self.p_mean_variance(x=x, t=t, s=s, alpha=alpha,jacobian=jacobian)
        else:
            model_mean, _, model_log_variance = self.p_mean_variance(x=x, t=t, s=s)
        noise = torch.randn_like(x)
        # no noise when t == 0
        nonzero_mask = (1 - (t == 0).float()).reshape(b, *((1,) * (len(x.shape) - 1)))
        return model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise *0.2 * self.random_sample

    # @torch.no_grad()
    def p_sample_loop(self, state, shape, alpha=None,critic=None, verbose=False, return_diffusion=False):
        ...
        for i in reversed(range(0, self.n_timesteps)):
            ...
            if not alpha == None:
                jacobian = compute_jacobian(critic,state,x).detach()
                x = self.p_sample(x, timesteps, state, alpha=alpha,jacobian=jacobian)
            else:
    ...
    def alpha_sample(self, state, alpha, critic, *args, **kwargs):
        batch_size = state.shape[0]
        shape = (batch_size, self.action_dim)
        action = self.p_sample_loop(state, shape, alpha=alpha, critic=critic, *args, **kwargs)
        return action.clamp_(-self.max_action, self.max_action)
    # ------------------------------------------ training ------------------------------------------#
