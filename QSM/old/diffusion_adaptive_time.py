...
class Diffusion(nn.Module):
    def __init__(self, state_dim, action_dim, model, max_action,
                 beta_schedule='linear', n_timesteps=1000,
    ...
    def p_sample(self, x, t, s):
        ...
        reverse_stop = False
        if model_mean.norm(dim=-1).mean() < 0.0001 :
            reverse_stop = True
        noise = torch.randn_like(x)
        # no noise when t == 0
        nonzero_mask = (1 - (t == 0).float()).reshape(b, *((1,) * (len(x.shape) - 1)))
        return model_mean + nonzero_mask * (0.5 * model_log_variance).exp() * noise *0.2 * self.random_sample,reverse_stop
    ...
    def p_sample_loop(self, state, shape, verbose=False, return_diffusion=False):
        ...
        for i in reversed(range(0, self.n_timesteps)):
            ...
            x, reverse_stop = self.p_sample(x, timesteps, state)
            if reverse_stop:
                break
            progress.update({'t': i})
        ...
        if return_diffusion:
            ...
            return x, self.n_timesteps - i

    # @torch.no_grad()
    def sample(self, state, *args, **kwargs):
        batch_size = state.shape[0]
        shape = (batch_size, self.action_dim)
        action,reverse_step = self.p_sample_loop(state, shape, *args, **kwargs)
        return action.clamp_(-self.max_action, self.max_action),reverse_step
