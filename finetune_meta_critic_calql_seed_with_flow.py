...
if cfg.log.wandb:
    ...
    run = wandb.init(
        ...
        name=f"{start_time_log}_meta_critic_calql_finetuning_seed_{str(cfg.train.random_seed)}",
        dir=f"wandb/meta_critic_calql_finetuning/seed{str(cfg.train.random_seed)}",
    )
...
val_logs = eval_policy(
    ...
    print_results=True,
    path=f"trajs/meta_critic_calql_finetuning/seed{str(cfg.train.random_seed)}/human{str(cfg.sim.human_num)}/{cfg.train.finetune_mode}/itr0/",
)
if cfg.log.wandb:
    wandb.log(
        {
    ...
    for i, ch in enumerate(pbar):
        ...
        if (i + 1) % cfg.eval.finetune_interval == 0:
            val_logs = eval_policy(
                ...
                render_type=cfg.eval.render_type,
                print_results=True,
                path=f"trajs/meta_critic_calql_finetuning/seed{str(cfg.train.random_seed)}/human{str(cfg.sim.human_num)}/{cfg.train.finetune_mode}/itr{str(i+1)}/",
            )
            if cfg.log.wandb:
                wandb.log(
                    {
