...
if cfg.log.wandb:
    ...
    run = wandb.init(
        ...
        name=f"{start_time_log}_calql_meta_critic_finetuning/seed{str(cfg.train.random_seed)}",
        dir=f"wandb/calql_meta_critic_finetuning/seed{str(cfg.train.random_seed)}",
    )
...
val_logs = eval_policy(
    ...
    print_results=True,
    path=f"trajs/calql_meta_critic_finetuning/seed{str(cfg.train.random_seed)}/human{str(cfg.sim.human_num)}/{cfg.train.finetune_mode}/itr0",
)
if cfg.log.wandb:
    wandb.log(
        {
    ...
    for i, ch in enumerate(pbar):
        ...
        if (i + 1) % cfg.eval.finetune_interval == 0:
            val_logs = eval_policy(
                ...
                render=cfg.eval.val_render,
                render_type=cfg.eval.render_type,
                path=f"trajs/calql_meta_critic_finetuning/seed{str(cfg.train.random_seed)}/human{str(cfg.sim.human_num)}/{cfg.train.finetune_mode}/itr{str(i+1)}",
                print_results=True,
            )
            if cfg.log.wandb:
                wandb.log(
